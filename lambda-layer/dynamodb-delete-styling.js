...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!photoId) {
      return response.error('photoId가 필요합니다.', origin, 400);
    }

    // userId를 String으로 변환
    const userId = String(user.userId);

    // DynamoDB에서 stylingPhotos.{photoId} 삭제 (REMOVE 사용)
    const updateCommand = new UpdateCommand({
      ...
      UpdateExpression: 'REMOVE stylingPhotos.#photoId SET updatedAt = :updatedAt',
    ...
    console.log('[DeleteStyling] Styling photo deleted from DynamoDB:', {
      userId,
      projectId,
      photoId,
    });
    ...
    return response.success(
      {
        message: '스타일링 사진이 삭제되었습니다.',
        photoId,
      },
      origin
    );
  } catch (error) {
    console.error('DeleteStyling Error:', error.message);
    return response.serverError('스타일링 사진 삭제 중 오류가 발생했습니다.', origin);
  }
};
